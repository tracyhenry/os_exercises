...
- 首先实现可显示字符串的bootloader，包括对CGA、串并口的操作，切换实模式到保护模式、创建GDT，加载Kernel
- 接着实现ELF文件格式的解析，使能中断，建立IDT

> 


如何能获取一个系统调用的调用次数信息？如何可以获取所有系统调用的调用次数信息？请简要说明可能的思路。(spoc)
- 在中断记录表的每一行记录被调用的次数，每次调用的时候，如果是系统调用，就将对应次数加1，同时维护所有变量的的和。
