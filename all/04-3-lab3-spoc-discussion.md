...
- 首先，将Page struct中的pra_page_link定义为clock的环形链表，并在Page struct中增加成员变量v代表访问位；再将mm_struct中的sm_priv作为指针，指向clock算法中的当前页。
- Clock算法和LRU都是最优算法的近似，由于CLOCK的贪心性质，缺页率可能略高于LRU。
- LRU在lab3完全可以实行，用链表模拟栈即可。
...
- 地址不对齐
- 缺页
- ucore的反应大多都是重启，有时候会提示缺页异常、从磁盘调页等等。
